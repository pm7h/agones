...
class UserBehavior(TaskSet):  
    ...
    def scaleUpFleet(self):
		...
		payload = {"apiVersion":"stable.agones.dev/v1alpha1","kind":"Fleet","metadata":{"generateName":"fleet-simple-udp", "namespace":"default"},"spec":{"replicas":initial_size, "scheduling":"Packed", "strategy": {"type": "RollingUpdate"},"template":{"spec":{"ports":[{"name":"default", "portPolicy": "dynamic", "containerPort": 26000}], "template":{"spec":{"containers":[{"name":"simple-udp","image":"gcr.io/agones-images/udp-server:0.5"}]}}}}}}
		...
		events.request_success.fire(request_type="fleet_spawn_up", name="fleet_spawn_up", response_time=total_time, response_length=0)	

		# Scale up the fleet.
		fleet_size = FLEET_SIZE
		resource_version = self.getResourceVersion(selfLink)
		start_time = time.time()
		payload = {"apiVersion":"stable.agones.dev/v1alpha1","kind":"Fleet","metadata":{"name":str(name), "namespace":"default", "resourceVersion":str(resource_version)},"spec":{"replicas":fleet_size, "scheduling":"Packed", "strategy": {"type": "RollingUpdate"},"template":{"spec":{"ports":[{"name":"default", "portPolicy": "dynamic", "containerPort": 26000}], "template":{"spec":{"containers":[{"name":"simple-udp","image":"gcr.io/agones-images/udp-server:0.5"}]}}}}}}
		response = self.client.put(selfLink, data=json.dumps(payload), headers=headers)
		self.waitForScaling(selfLink, fleet_size)
		total_time = int((time.time() - start_time) * 1000)
		events.request_success.fire(request_type="fleet_scaling_up", name="fleet_scaling_up", response_time=total_time, response_length=0)

		# Scale down the fleet.
		resource_version = self.getResourceVersion(selfLink)
		start_time = time.time()
		payload = {"apiVersion":"stable.agones.dev/v1alpha1","kind":"Fleet","metadata":{"name":str(name), "namespace":"default", "resourceVersion":str(resource_version)},"spec":{"replicas":0, "scheduling":"Packed", "strategy": {"type": "RollingUpdate"},"template":{"spec":{"ports":[{"name":"default", "portPolicy": "dynamic", "containerPort": 26000}], "template":{"spec":{"containers":[{"name":"simple-udp","image":"gcr.io/agones-images/udp-server:0.5"}]}}}}}}
		response = self.client.put(selfLink, data=json.dumps(payload), headers=headers)
